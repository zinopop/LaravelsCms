                    ...
                    <form role="form" id="search_form">
                        <div class="form-inline">
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">运输方式代码</label>
                                <input type="text" name="code" class="form-control">
                            </div>
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">运输方式名称</label>
        ...
        var tableObj = function () {
            ...
            _tableObj.init = function () {
                $('#tb_departments').bootstrapTable({
                    url: '{{ route('admin.parameter.transportation.getData') }}',         //请求后台的URL（*）
                    method: 'post',                      //请求方式（*）
            ...
            _buttonObj.init = function () {
                $("#add").click(function () {
                    layer.open({
                        ...
                        area: ['700px', '500px'],
                        type: 2,
                        content: '{{ route('admin.parameter.transportation.detail') }}',
                        btn: ['保存', '关闭'],
                        closeBtn:2,
                        maxmin:true,
                ...
                $("#delete").click(function () {
                    ...
                    layer.confirm('是否删除？', {
                        ...
                        if(row.length > 0 ){
                            ...
                            $.ajax({
                                method: 'post',
                                url: '{{ route('admin.parameter.transportation.del') }}',
                                data: {
                                    _token:'{{ csrf_token() }}',
                                    idArray:idArray
                                },
        ...
        var editObj = function () {
            ...
            _editObj.init = function (primaryKey) {
                layer.open({
                    ...
                    area: ['700px', '500px'],
                    type: 2,
                    content: '{{ route('admin.parameter.transportation.detail') }}?id='+primaryKey,
                    btn: ['保存', '关闭'],
                    closeBtn:2,
                    maxmin:true,
                    yes: function(index, layero){
        ...
        * */
        var editForPramkey = function () {
            ...
            _editForPramkey.init = function (obj,index) {
                ...
                $.ajax({
                    method: 'post',
                    url: '{{ route('admin.parameter.transportation.save') }}',
                    data: {
                        enable:obj.value,
                        id:row.id,
                        _token:'{{ csrf_token() }}',
