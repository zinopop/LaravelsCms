                    ...
                    <form role="form" id="search_form">
                        <div class="form-inline">
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">省代码</label>
                                <input type="text" name="provincial_code" class="form-control">
                            </div>
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">省名称</label>
                                <input type="text" name="provincial_name" class="form-control">
                            </div>
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">货源地代码</label>
                                <input type="text" name="source_code" class="form-control">
                            </div>
                            <div class="form-group m-r-xs">
                                <label for="exampleInputEmail2">货源地名称</label>
        ...
        var tableObj = function () {
            ...
            _tableObj.init = function () {
                $('#tb_departments').bootstrapTable({
                    url: '{{ route('admin.parameter.destianation.getData') }}',         //请求后台的URL（*）
                    method: 'post',                      //请求方式（*）
            ...
            _tableObj.queryParams = function (params) {
                var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
                    ...
                    provincial_code:$('#search_form').find("input[name='provincial_code']").val(),
                    provincial_name:$('#search_form').find("input[name='provincial_name']").val(),
                    source_code:$('#search_form').find("input[name='source_code']").val(),
                    source_name:$('#search_form').find("input[name='source_name']").val(),
                    _token:'{{ csrf_token() }}'
                };
                return temp;
            };
            ...
            _buttonObj.init = function () {
                $("#add").click(function () {
                    layer.open({
                        ...
                        area: ['700px', '500px'],
                        type: 2,
                        content: '{{ route('admin.parameter.destianation.detail') }}',
                        btn: ['保存', '关闭'],
                        closeBtn:2,
                        maxmin:true,
                ...
                $("#delete").click(function () {
                    ...
                    layer.confirm('是否删除？', {
                        ...
                        if(row.length > 0 ){
                            ...
                            $.ajax({
                                method: 'post',
                                url: '{{ route('admin.parameter.destianation.del') }}',
                                data: {
                                    _token:'{{ csrf_token() }}',
                                    idArray:idArray
                                },
        ...
        var editObj = function () {
            ...
            _editObj.init = function (primaryKey) {
                layer.open({
                    ...
                    area: ['700px', '500px'],
                    type: 2,
                    content: '{{ route('admin.parameter.destianation.detail') }}?id='+primaryKey,
                    btn: ['保存', '关闭'],
                    closeBtn:2,
                    maxmin:true,
                    yes: function(index, layero){
